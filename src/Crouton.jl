...
- hatchdir: direction of the bottommost hatch lines
...
function createconfig(filename="config.jl")
    config = """Dict(
        ...
        :maxseglength => 25u"µm",
        :keygap => 20u"µm",
        :dhammockhatch => 300u"nm",
        :dhatch => 300u"nm",
        :laserpower => 50u"mW",
        :scanspeed => 50_000u"µm/s",
        :stagespeed => 100u"µm/s",
        :interfacepos => 10u"µm",
        :hamscanspeed => 50_000u"µm/s",
...
#=============Don't need this now that we're writing individual hammocks==================
...
function scaffold(scaffolddir,kwargs::Dict)
    ...
    kernels = cd(scaffolddir) do
        ...
        b = bumper(p,bec,kwargs[:bumperseglength],zero(kwargs[:hbottom]),
        ...
        kcvec = vcat(map(ringvec) do ring
                         collect(ring)
                     end...)
        #====Not anymore======================
        #we want to print half the scaffolds while spiraling in and then half when spiraling out
        (oddk,evenk) = map([isodd,iseven]) do f
            withindices = filter(enumerate(kcvec) |> collect) do (i,_)
                f(i)
            end
            #strip the indices
            map(withindices) do (_,x)
                x
            end
        end
        ====================================#
        @info "$(length(kcvec)) total kernels"
        for (j,kernelcenter) in enumerate(reverse(kcvec))
            @info "kernel $j"
            ...
            hamblock = map(ourhams) do hc
                #get the vertices which will be perfectly tangent with the bottom of the beams
                hexcenter = center(hc)
                verts = map(hc.verts) do v
                    scrootchvert(v,hexcenter,0.5*kwargs[:wpost]/cosd(30))
                end

                #now scrootch for overlap
                verts = map(verts) do v
                    scrootchvert(v,hexcenter,-kwargs[:hamoverlap]/cosd(30))
                end
                
                slicepairs = map(range(start=kwargs[:hbottom]+kwargs[:hbeam],
                                        step=-kwargs[:dhammockslice],
                                        length=kwargs[:nhammock])) do z
                                            #distance above bottom of beams
            ...
            numham = length(hamblock)
            hamblock = if numham == 1
                hamblock[1]
            elseif iszero(numham)
                hamblock
            else
                merge(hamblock...)
            end
            #hatch
            @info "hatching kernel $j"
            hatched = hatch(postbeams,dhatch=kwargs[:dhatch],
                            bottomdir=kwargs[:hatchdir],diroffset=pi/2)
